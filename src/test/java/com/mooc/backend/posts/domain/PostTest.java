...
class PostTest {
    ...
    void createAssignsIdAndDefaults() {
        Post post = Post.create(AUTHOR, "Title", "Body", null, List.of(), PostStatus.DRAFT, NOW);
    ...
    void updateReplacesProvidedFieldsAndTouchesUpdatedAt() {
        ...
        post.update("NewTitle", "NewBody", "http://x/y.png",
                List.of("a", "b"), PostStatus.PUBLISHED, later);
    ...
    void isPublishedReflectsStatus() {
        Post draft = Post.create(AUTHOR, "T", "c", null, List.of(), PostStatus.DRAFT, NOW);
        Post published = Post.create(AUTHOR, "T", "c", null, List.of(), PostStatus.PUBLISHED, NOW);
